...
Uses NVIDIA NIM API. Includes regeneration for auto-fix.
"""
...
def generate_code(plan: dict[str, Any], profile: dict) -> str:
    ...
    code = call_llm(prompt=user_msg, system=_SYSTEM_PROMPT, temperature=0.15)
...
def regenerate_code(
    ...
    code = call_llm(prompt=user_msg, system=_SYSTEM_PROMPT, temperature=0.2)
    code = _strip_fences(code)
