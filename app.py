"""
app.py — Streamlit Dashboard for AI Data Analyst Agent

A premium, interactive web UI for the agent system.
...
from utils import get_logger, check_llm_available, LLM_MODEL
...
with col_status1:
    nvidia_ok = check_llm_available()
    if nvidia_ok:
        st.markdown('<p class="status-ok">✅ NVIDIA NIM: Connected</p>', unsafe_allow_html=True)
    else:
        st.markdown('<p class="status-err">❌ NVIDIA API Key not configured</p>', unsafe_allow_html=True)

with col_status2:
    if nvidia_ok:
        st.markdown(f'<p class="status-ok">✅ Model {LLM_MODEL}: Ready</p>', unsafe_allow_html=True)
...
if run_clicked and query:
    if not check_llm_available():
        st.error("❌ NVIDIA API key is missing. Set the NVIDIA_API_KEY environment variable.")
        st.stop()
